...
const weeks = planningWeeks(2026, 2)
...
describe('solvePlanning', () => {
  it('places non-conflicting requests exactly as requested', () => {
    ...
    const requests: VacationRequest[] = [
      { id: 'r1', employeeId: 'e1', startWeek: 10, endWeek: 11, priority: 'normal' },
      { id: 'r2', employeeId: 'e2', startWeek: 20, endWeek: 20, priority: 'normal' },
    ]
    const result = solvePlanning(weeks, employees, requests, constraints)
  ...
  it('always honors fixed requests even if they violate concurrency, and warns', () => {
    ...
    const requests: VacationRequest[] = [
      { id: 'r1', employeeId: 'e1', startWeek: 5, endWeek: 5, priority: 'fixed' },
      { id: 'r2', employeeId: 'e2', startWeek: 5, endWeek: 5, priority: 'fixed' },
      { id: 'r3', employeeId: 'e3', startWeek: 5, endWeek: 5, priority: 'fixed' },
    ]
    const result = solvePlanning(weeks, employees, requests, constraints)
    expect(result.assignments.every((a) => a.status === 'as-requested')).toBe(true)
  ...
  it('finds the nearest alternative when the exact week is blocked by concurrency', () => {
    const employees = makeEmployees(['A', 'B', 'C'])
    const requests: VacationRequest[] = [
      { id: 'r1', employeeId: 'e1', startWeek: 10, endWeek: 10, priority: 'fixed' },
      { id: 'r2', employeeId: 'e2', startWeek: 10, endWeek: 10, priority: 'fixed' },
      { id: 'r3', employeeId: 'e3', startWeek: 10, endWeek: 10, priority: 'normal' },
    ]
    const result = solvePlanning(weeks, employees, requests, constraints)
  ...
  it('respects max consecutive vacation weeks', () => {
    ...
    const requests: VacationRequest[] = [
      { id: 'r1', employeeId: 'e1', startWeek: 1, endWeek: 2, priority: 'normal' },
      { id: 'r2', employeeId: 'e1', startWeek: 3, endWeek: 3, priority: 'normal' },
    ]
    const result = solvePlanning(weeks, employees, requests, constraints)
  ...
  it('respects individual employee vacation week cap', () => {
    ...
    const requests: VacationRequest[] = [
      { id: 'r1', employeeId: 'e1', startWeek: 1, endWeek: 2, priority: 'normal' },
      { id: 'r2', employeeId: 'e1', startWeek: 10, endWeek: 10, priority: 'normal' },
    ]
    const result = solvePlanning(weeks, employees, requests, constraints)
    const second = result.assignments.find((a) => a.requestId === 'r2')!
  ...
  it('marks a request unresolved when no alternative exists in the whole year', () => {
    ...
    for (let w = 1; w <= totalWeeks; w++) {
      requests.push({ id: `fa${w}`, employeeId: 'e1', startWeek: w, endWeek: w, priority: 'fixed' })
    }
    requests.push({ id: 'rc', employeeId: 'e3', startWeek: 30, endWeek: 30, priority: 'normal' })
    const withB = [...requests]
    for (let w = 1; w <= totalWeeks; w++) {
      withB.push({ id: `fb${w}`, employeeId: 'e2', startWeek: w, endWeek: w, priority: 'fixed' })
    }
    const result = solvePlanning(weeks, employees, withB, { maxConcurrentEmployees: 2, maxConsecutiveWeeks: 60 })
    const rc = result.assignments.find((a) => a.requestId === 'rc')!
    expect(rc.status).toBe('unresolved')
  })

  it('covers a February-to-January period across ISO years', () => {
    expect(weeks[0]).toMatchObject({ isoYear: 2026, isoWeek: 5 })
    expect(weeks.at(-1)).toMatchObject({ isoYear: 2027, isoWeek: 4 })
